...
export default function HeroSlider({ sliders, reservationUrl }: HeroSliderProps) {
    ...
    <section className="relative h-screen min-h-[700px] flex items-center justify-center overflow-hidden">
      ...
      {loaded && (
        <div className="relative z-[2] text-center text-white max-w-[850px] px-8">
          <div className="hero-fade-1 text-sm font-medium tracking-[6px] uppercase text-accent mb-6">
            Kuşadası &bull; Ege&apos;nin İncisi
          </div>
          ...
          <p className="hero-fade-3 text-lg font-light leading-8 text-white/85 mb-11 max-w-xl mx-auto">
            {sliders[current]?.subtitle ||
              "Ege'nin büyüleyici kıyılarında, tarihin ve denizin buluştuğu eşsiz bir konaklama deneyimi sizi bekliyor."}
          </p>
          <div className="hero-fade-4 flex gap-5 justify-center flex-wrap">
            <a
              ...
              className="px-10 py-4 bg-accent text-white text-xs font-semibold tracking-[3px] uppercase hover:bg-accent-dark hover:translate-y-[-2px] hover:shadow-[0_8px_25px_rgba(201,169,110,0.35)] transition-all"
            >
              Rezervasyon Yap
            </a>
            <a
              href="/odalar"
              className="px-10 py-4 border-[1.5px] border-white text-white text-xs font-semibold tracking-[3px] uppercase hover:bg-white hover:text-primary transition-all"
            >
              Odalarımız
            </a>
          </div>
        ...
        <div className="absolute bottom-10 right-10 z-[2] hidden md:flex gap-2.5">
          {sliders.map((_, i) => (
            <button
              ...
              className={`h-[3px] transition-all cursor-pointer ${
                i === current
                  ? "w-[60px] bg-accent"
