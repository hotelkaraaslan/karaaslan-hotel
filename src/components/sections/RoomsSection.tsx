      ...
      <div className="max-w-[1200px] mx-auto px-8">
        <ScrollReveal className="text-center mb-16">
          ...
          <p className="text-text-light text-base leading-8 max-w-[650px] mx-auto">
            Her biri özenle tasarlanmış odalarımızda konfor ve zarafeti bir arada
            yaşayın. Deniz manzaralı odalarımız ile Ege&apos;nin büyüsüne uyanın.
          </p>
        </ScrollReveal>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          {rooms.map((room, i) => (
            <ScrollReveal key={room.id} delay={i * 150}>
              <Link href={`/odalar/${room.slug}`} className="block group bg-white overflow-hidden">
                ...
                <div className="p-8">
                  ...
                  <p className="text-sm text-text-light leading-7 mb-5">
                    {room.short_description || room.description}
                  </p>
                  <span className="inline-flex items-center gap-2.5 text-xs font-semibold tracking-[2px] uppercase text-accent group-hover:gap-4 transition-all">
                    Detayları İncele
                    <ArrowRight size={18} />
                  </span>
                </div>
