...
export default async function GaleriPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const [dict, gallery] = await Promise.all([getDictionary(lang as Locale), getGallery()]);

  return (
    <main>
      <PageHero title={dict.gallery.label} subtitle={dict.gallery.subtitle} imageUrl="https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?w=1920&q=80" />
