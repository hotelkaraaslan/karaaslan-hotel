...
export async function generateMetadata({ params }: { params: Promise<{ lang: string; slug: string }> }) {
  ...
  return { title: `${venue.title} | Hotel By Karaaslan Inn`, description: venue.short_description || venue.description };
}
            ...
            {venue.gallery_images.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {venue.gallery_images.map((img, i) => (
                  <div key={i} className="relative h-[200px] md:h-[250px]"><Image src={img} alt={`${venue.title} ${i + 1}`} fill className="object-cover" sizes="33vw" /></div>
                ))}
              </div>
            )}
          </ScrollReveal>
